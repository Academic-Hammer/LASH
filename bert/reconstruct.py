...
def construct(dataset, mode, original_index):
    ...
    for i in tqdm([0, 1]):
        path_1 = f'data/{args["dataset"]}/{args["mode"]}/bert_embedding/{args["mode"]}_embd128_{i}.pkl'
        path_2 = f'data/{args["dataset"]}/{args["mode"]}/bert_embedding/{args["mode"]}_idx128_{i}.pkl'
    ...
    with open(f'data/{args["dataset"]}/{args["mode"]}/bert_embedding/embeddings_new128', 'wb') as f:
        joblib.dump(dataset, f)

if __name__ == "__main__":
    args = vars(parser_args())
    original_index = read_index(f'data/{args["dataset"]}/{args["mode"]}/content_{args["mode"]}')
    with open(f'data/{args["dataset"]}/{args["mode"]}/bert_embedding/indices_new128', 'wb') as f:
